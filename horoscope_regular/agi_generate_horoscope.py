...
def agi_get_birthdate():
    if len(sys.argv) != 2:
        print("Usage: AGI <script> <birthdate>")
        sys.exit(1)
    birth_date_str = sys.argv[1]
    if len(birth_date_str) != 8:
        print("Invalid birthdate format. Expected YYYYMMDD.")
        sys.exit(1)
...
# Get birthdate from AGI input
birth_date_str = agi_get_birthdate()
...
# Generate and print the horoscope for Asterisk
horoscope = generate_horoscope()
print(f"SET VARIABLE horoscope \"{horoscope}\"")
